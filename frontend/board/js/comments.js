...
export async function renderComments(postId) {
    ...
    try {
        const comments = await apiFetch(`/api/board/posts/${encodeURIComponent(postId)}/comments`);
...
window.reportComment = async function (postId, commentId, commentText) {
    ...
    if (reason) {
        try {
            await apiFetch(`/api/board/posts/${encodeURIComponent(postId)}/comments/${encodeURIComponent(commentId)}/report`, {
                method: "POST",
                body: JSON.stringify({
...
window.editComment = async function (postId, commentId, currentText) {
    ...
    if (newText && newText !== currentText) {
        try {
            await apiFetch(`/api/board/posts/${encodeURIComponent(postId)}/comments/${encodeURIComponent(commentId)}`, {
                method: "PATCH",
                body: JSON.stringify({
                    text: newText,
...
window.openInquiry = async function () {
    ...
    if (inquiryText) {
        try {
            const u = getCurrentUser();
            await apiFetch("/api/board/inquiries", {
                method: "POST",
                body: JSON.stringify({
                    text: inquiryText,
                    user_name: displayNameFromUser(u),
