...
async function loadMyReplies(uid) {
    ...
    try {
        const rows = await apiFetch(`/api/questions?questioner_uid=${encodeURIComponent(uid)}&limit=50`);
        renderReplies(rows || []);
    } catch (e) {
        console.error(e);
        if (el) {
            el.innerHTML =
                '<p class="qb-hub-replies-empty">読み込みに失敗しました。Go API が起動しているか、ログイン状態を確認してください。</p>';
        }
    }
}
