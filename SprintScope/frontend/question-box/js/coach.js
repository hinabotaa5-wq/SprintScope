...
async function loadQuestions() {
    ...
    try {
        ...
        grid.innerHTML = `
            ...
            <p class="coach-load-hint">Go API（<code>GET /api/questions</code>）が起動しているか、<code>SPRINT_API_BASE</code> の設定を確認してください。Supabase の <code>questions</code> テーブルに行があるのに空の場合はバックエンドのログを確認してください。</p>`;
    }
}
...
function renderQuestions() {
    ...
    if (list.length === 0) {
        if (allQuestions.length > 0) {
            grid.innerHTML =
                `<p class="coach-grid-empty">API から <strong>${allQuestions.length}件</strong> 読み込み済みですが、いまのプラン・フィルターに合う質問がありません。「すべて」プラン・「すべて」種類に切り替えてください。</p>`;
        } else {
            grid.innerHTML =
                "<p class=\"coach-grid-empty\">まだ1件も取得できていません。質問者側で送信が成功しているか、Go API と Supabase の <code>questions</code> テーブルを確認してください。</p>";
        }
        return;
    }
