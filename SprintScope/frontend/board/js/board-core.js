...
const COMPRESSION_TARGET_BYTES = 25 * 1024 * 1024;
const COMPRESSED_MAX_WIDTH = 960;
const COMPRESSED_VIDEO_BITS_PER_SECOND = 1_600_000;

function isMobileLayout() {
    return window.matchMedia(MOBILE_LAYOUT_BREAKPOINT).matches;
}

async function compressVideoForUpload(file) {
    if (!window.MediaRecorder || !file || file.size <= COMPRESSION_TARGET_BYTES) {
        return file;
    }
    ...
    try {
        await new Promise((resolve, reject) => {
            ...
            video.onerror = () => reject(new Error("動画メタデータの取得に失敗しました"));
        });

        const metadataWidth = Math.max(1, video.videoWidth || 1);
        const metadataHeight = Math.max(1, video.videoHeight || 1);
        const scale = Math.min(1, COMPRESSED_MAX_WIDTH / metadataWidth);
        const targetWidth = Math.max(2, Math.round(metadataWidth * scale));
        const targetHeight = Math.max(2, Math.round(metadataHeight * scale));
        ...
        const recorder = new MediaRecorder(stream, {
            ...
            videoBitsPerSecond: COMPRESSED_VIDEO_BITS_PER_SECOND,
        });
        ...
        const compressionDone = new Promise((resolve, reject) => {
            recorder.onerror = () => reject(new Error("動画圧縮に失敗しました"));
            recorder.onstop = () => {
        ...
        await new Promise((resolve, reject) => {
            ...
            video.onerror = () => reject(new Error("動画再生中にエラーが発生しました"));
        });

        if (recorder.state !== "inactive") recorder.stop();
        const compressed = await compressionDone;
        return compressed.size < file.size ? compressed : file;
    } catch (error) {
        console.warn("動画圧縮をスキップ:", error);
        return file;
    } finally {
        video.pause();
...
window.submitPost = async function () {
    ...
    const loadingText = document.querySelector("#loading-msg .loading-text");
    if (loadingText) loadingText.textContent = "動画を最適化中...";

    uploadFile = await compressVideoForUpload(originalFile);
    if (loadingText) loadingText.textContent = "アップロード中...";

    try {
        const data = await apiUploadVideo(uploadFile, "auto_delete_90d");
...
export async function renderVideos() {
    ...
    try {
        const sort = currentSortOrder === "newest" ? "newest" : "oldest";
        const rows = await apiFetch(`/api/board/posts?sort=${encodeURIComponent(sort)}`);
...
window.reportVideo = async function (postId) {
    ...
    if (reason) {
        try {
            await apiFetch(`/api/board/posts/${encodeURIComponent(postId)}/reports`, {
                method: "POST",
                body: JSON.stringify({
                    reason: reason,
                }),
...
window.deleteVideo = async function (id) {
    ...
    try {
        await apiFetch(`/api/board/posts/${encodeURIComponent(id)}`, { method: "DELETE" });
