...
/** qb-data.js と同じ Cloudinary（回答動画アップロード用） */
...
async function loadQuestions() {
    ...
    if (!client) {
        grid.innerHTML = "<p>Supabase が初期化されていません。ページを再読み込みしてください。</p>";
        return;
    }
...
async function submitCoachAdvice() {
    ...
    try {
        ...
        const { error } = await client.from("questions").update(payload).eq("id", activeQuestion.id);
        ...
        Swal.fire({
            icon: "error",
            title: "保存に失敗しました",
            html: `${escapeHtml(e.message || String(e))}<p style="margin-top:0.75rem;font-size:0.85rem;color:#78716c;text-align:left">Supabase に <code>coach_advice_text</code> / <code>coach_advice_video_url</code> カラムと <code>questions_update_anon</code> ポリシーがあるか、<code>supabase_questions.sql</code> を確認してください。</p>`,
            confirmButtonColor: "#2f8a96"
        });
    } finally {
        if (btn) {
