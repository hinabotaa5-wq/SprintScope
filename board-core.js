...
async function promptLoginMethod() {
    ...
    if (methodResult.isDenied) {
        ...
        const otpResult = await Swal.fire({
            title: "認証コードを入力",
            html: `
                <p style="margin-bottom: 10px;">メールで届いた6桁のコードを入力してください。</p>
                <input id="email-otp-input" class="swal2-input" inputmode="numeric" maxlength="6" placeholder="123456" autocomplete="one-time-code" />
            `,
            focusConfirm: false,
            showCancelButton: true,
