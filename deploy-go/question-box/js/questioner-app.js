...
async function qbApiFetch(path, options = {}) {
  ...
  if (!response.ok) {
    console.error("[qbApiFetch] request failed", {
      method,
      path,
...
async function qbUploadVideo(file, tags = "auto_delete_90d") {
  const client = window.qbSupabase;
  let token = "";
  if (client?.auth?.getSession) {
    const { data } = await client.auth.getSession();
  ...
  const headers = {};
  if (token) headers.Authorization = `Bearer ${token}`;
  const response = await fetch(`${API_BASE}/api/uploads/video`, {
    method: "POST",
    headers,
    body: form,
  });
  ...
  if (!response.ok) {
    console.error("[qbUploadVideo] request failed", {
      status: response.status,
      responseBody: text,
    });
    throw new Error(json?.message || `HTTP ${response.status}`);
...
async function submitToCoach() {
  ...
  if (format === "plus" && file) {
    try {
      const data = await qbUploadVideo(file, "auto_delete_90d");
  ...
  try {
    await qbApiFetch("/api/questions", {
      method: "POST",
